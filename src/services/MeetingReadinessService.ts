...
export class MeetingReadinessService {
  static evaluate(leadTexts: string[], qualification: any, attribution?: { launchId: string; participantId: string }): MeetingReadiness {
    ...
    if (leadTexts.length >= 2 && evidence.has('declared_interest') && evidence.has('declared_need_or_goal')) evidence.add('discovery_conversation');
    ...
    console.log('MEETING READINESS', {
  evidence: [...evidence],
  leadTexts: leadTexts.length,
  normalizedIntent,
});
