...
const actions = ['create_or_update_lead', 'add_tag', 'change_status', 'update_score', 'generate_ai_response', 'create_proposal', 'create_task', 'suggest_followup', 'mark_meeting_candidate', 'add_note', 'wait', 'send_message'];
...
export class AutomationService {
  ...
  static sanitize(input: any, partial = false) {
    ...
    if (!partial || input.actions != null) { if (!Array.isArray(input.actions) || !input.actions.length || input.actions.length > 30) throw new Error('Debe existir entre 1 y 30 acciones'); output.actions = input.actions.map((item: any) => { const type = item.type === 'delay' ? 'wait' : item.type; item.type === 'delay' ? 'wait' : item.type; if (!actions.includes(type)) throw new Error(`Acción no permitida: ${type}`); const actionConditions = Array.isArray(item.conditions) ? item.conditions.map((condition: any) => { if (!fields.includes(condition.field) || !operators.includes(condition.operator)) throw new Error('Condición de acción no permitida'); return { field: condition.field, operator: condition.operator, value: condition.value }; }) : []; return { type, config: typeof item.config === 'object' && item.config ? item.config : item.message ? { message: String(item.message).slice(0, 1000) } : item.delay != null ? { durationMs: Number(item.delay) } : {}, conditions: actionConditions }; }); }
    return output;
  }
