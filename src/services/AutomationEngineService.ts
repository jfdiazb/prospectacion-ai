...
export class AutomationEngineService {
  ...
  if (!configuredMessage) throw new Error('send_message requiere un mensaje');
  ...
  const result = await MessagingService.send({
    ...
    text: String(configuredMessage).slice(0, 1000),
    recipient: event.recipient,
  });

  return { status: result, autoSent: result === 'sent' };
}
    if (
  ...
  recipient: event.recipient,
      ...
      return { proposalId: proposal._id, status: result.status, autoSent: true };
    }
