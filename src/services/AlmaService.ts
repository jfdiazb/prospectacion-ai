...
export class AlmaService {
  ...
  static async processMessage(context: AlmaContext): Promise<string> {
    ...
    console.log('MEETING READINESS RESULT', meetingReadiness);
    ...
    if (!isRejected) {
      ...
      await TaskService.upsertPendingFollowUp(context.userId, {
        ...
        metadata: { intent, score, autoGenerated: true, meetingReadiness: meetingReadiness.reason, ...LaunchAttributionService.metadata(launchAttribution) },
      });
    }
