...
describe('Auth integration tests', () => {
  ...
  test('completes the automatic WhatsApp offer and Calendly acceptance flow with historical state', async () => {
    ...
    await postAutomaticMessage('wamid.automatic-qualified-history', 'No me interesa esta propuesta por ahora');
    const existingLead: any = await Lead.findOne({ userId: owner!._id, phone: '573006667777' });
    const existingConversation: any = await Conversation.findOne({ userId: owner!._id, leadId: existingLead._id });
    const syntheticHistory = Array.from({ length: 294 }, (_, index) => ({
      sender: index % 2 === 0 ? 'lead' : 'ai',
      text: index % 2 === 0 ? `Contexto histórico ${index}` : `Pregunta histórica ${index}`,
      platform: 'whatsapp',
      direction: index % 2 === 0 ? 'inbound' : 'outbound',
      status: index % 2 === 0 ? 'received' : 'simulated',
      timestamp: new Date(Date.now() - (294 - index) * 60000),
    }));
    await Conversation.updateOne({ _id: existingConversation._id }, {
      $set: {
        messages: [...syntheticHistory, ...existingConversation.messages.map((message: any) => message.toObject())],
        aiAskedTopics: ['desired_outcome', 'main_obstacle', 'previous_attempts', 'support_needed'],
        aiMemoryInitializedAt: new Date(),
      },
    });
    expect((await Conversation.findById(existingConversation._id))!.messages).toHaveLength(296);
    await Meeting.create({ userId: owner!._id, leadId: existingLead._id, conversationId: existingConversation._id, provider: 'calendly', status: 'pending_booking', topic: 'Reserva anterior', sourceEventId: 'historical-booking', bookingUrl: 'https://calendly.com/example/existing', bookingToken: 'historical-token' });
    ...
    await postAutomaticMessage(eventId, text);
    ...
    expect(await Meeting.findOne({ userId: owner!._id })).toMatchObject({ status: 'pending_booking', sourceEventId: 'historical-booking' });
    ...
    const outboundCountBeforeOptOut = await OutboundMessage.countDocuments({ userId: owner!._id });
    await postAutomaticMessage('wamid.automatic-qualified-optout', 'No quiero continuar, no me contactes');
    await postAutomaticMessage('wamid.automatic-qualified-after-optout', text);
    expect(await Lead.findById(existingLead._id)).toMatchObject({ status: 'rejected', tags: expect.arrayContaining(['opt_out']) });
    expect(await OutboundMessage.countDocuments({ userId: owner!._id })).toBe(outboundCountBeforeOptOut);
    process.env.WHATSAPP_REPLY_MODE = 'assisted';
    process.env.WHATSAPP_AUTO_REPLY_ENABLED = 'false';
  });
